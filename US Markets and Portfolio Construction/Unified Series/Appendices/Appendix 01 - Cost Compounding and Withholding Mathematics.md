...
### Companion to Unified Parts 2 and 3: wrapper cost, withholding and cross-border implementation
...
Most investors build a position through periodic contributions rather than a single lump sum. Equation 1.1 extends to a level monthly contribution PMT using the standard future-value-of-an-annuity formula, with the annual net rate (r minus e) converted to an equivalent monthly rate i = (1 + r minus e)^(1/12) minus 1:

\[
FV = PMT \times \frac{(1+i)^{m}-1}{i}
\]  (Equation 1.3)
...
|---|---|---|---|
| SPYM | 0.02% | $283,846 | Not applicable |
...
\[
Net dividend = Gross dividend \times (1 - w)
\]  (Equation 1.2)
...
Because the gap scales linearly with both position size and yield, an investor can approximate their own annual gap as (position multiplied by yield multiplied by 0.15) without needing the full table , the 15-percentage-point rate difference (30% minus 15%) applied directly to the gross dividend.
...
Applying the schedule: tax on $60,000 = $8,200 + 0.24 multiplied by ($60,000 minus $40,000) = $8,200 + $4,800 = $13,000 , exactly the unified credit available to a nonresident alien's estate. This is why $60,000 functions as an effective exemption even though the statute frames it as a credit rather than an exemption.
