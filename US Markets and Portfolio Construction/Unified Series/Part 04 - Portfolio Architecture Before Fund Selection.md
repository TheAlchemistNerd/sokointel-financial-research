# Part 04: Portfolio Architecture Before Fund Selection

*US Markets and Portfolio Construction, unified nine-part series. Research edition: 15 September 2026.*

Before comparing funds, define the job the portfolio must do. This chapter develops strategic core, core-plus, core-satellite, factor tilt, barbell, tactical and high-conviction variants as distinct governance choices. The directional nested-ring visual represents a hierarchy of decision rights, not a recommended allocation.

![Directional rings representing a portfolio core, bounded extensions, deliberate tilts and small satellites.](/static/site/us-strategy-rings-directional.png "Portfolio strategy layers")

| Visual layer, centre outward | Portfolio role | Governance question |
| --- | --- | --- |
| Core | Broad, low-cost market exposure | What long-horizon exposure is indispensable? |
| Core-plus | A bounded change within the core | Why is this still part of the foundation? |
| Deliberate tilt | A stated preference with a measurable size | What evidence would reverse the preference? |
| Factor tilt | Exposure to a documented return characteristic | Which factor, implementation and tracking error are accepted? |
| Satellite | A small, separately accountable conviction | What loss and concentration budget applies? |

## Series scope

Part 1 established a practical distinction between a **broad core** and a **deliberate tilt**. The next question is more structural: if core-satellite and core-plus are only two ways to organize a portfolio, what other architectures exist, and when do they make sense?

Asset classes use investment vocabulary in different ways. Terms such as **core, core-plus, value-add and opportunistic** are widely used in private real estate and infrastructure to describe increasing levels of leverage, operating intensity and business-plan risk. In a liquid ETF portfolio, however, the more useful architecture labels are things such as **strategic allocation, core-satellite, factor tilt, barbell, concentrated/high-conviction, risk parity, portable alpha and tactical allocation**. Reusing a real-estate risk ladder as if it were a universal equity taxonomy can create false precision.

The objective is to identify the economic logic behind each architecture, show the mathematics that makes the differences visible, and build rules for deciding whether a new sleeve genuinely changes the portfolio.
...
![A map of portfolio architectures, showing the relationship among strategic core, extensions, tilts and satellites.](/static/site/us-architecture-map.png "Portfolio architecture map")
...
### A bond sleeve changes portfolio behaviour

A bond allocation is not merely a lower-return version of equities. It introduces contractual cash flows, interest-rate sensitivity, credit risk and, in many portfolio designs, a source of liquidity for known spending. A broad aggregate-bond ETF such as AGG or BND can be an implementation vehicle for that role, but duration, credit quality and the investor's withdrawal horizon still determine whether the sleeve is suitable [13], [14].

| Portfolio role | What the bond sleeve is intended to do | Main risk to monitor |
| --- | --- | --- |
| Liquidity reserve | Fund spending due in the next few years | Credit quality, duration and forced-sale risk |
| Diversifier | Reduce dependence on correlated equity outcomes | Correlation can rise in stressed markets |
| Income sleeve | Provide contractual interest payments | Yield changes and reinvestment risk |
| Rebalancing source | Supply capital after an equity decline | A bond fund can also decline when yields rise |

In a two-sleeve illustration, a 70% equity allocation with 16% volatility and a 30% bond allocation with 6% volatility has a portfolio volatility of about 11.52% when their correlation is 0.10. The result comes from covariance, not from averaging the two volatilities. Appendix 2 shows the calculation and the sensitivity to the correlation assumption.
...
The illustrative policies can use a U.S. core using SPYM and QQQM, plus satellites such as semiconductor ETFs and an options wheel. That can be coherent if the jobs are explicit. SPYM is broad beta; QQQM is a growth tilt; a semiconductor ETF is a narrower sector tilt; the wheel is a payoff-management strategy. The architecture fails if every satellite is justified by the same thesis, “technology will outperform,” because apparent sleeve diversification masks a single underlying bet.
...
## 12. Applying the framework to an illustrative U.S. portfolio
...
[13] BlackRock, "iShares Core U.S. Aggregate Bond ETF (AGG)," 2026. [Online]. Available: https://www.blackrock.com/us/individual/products/239458/ishares-core-total-us-bond-market-etf. [Accessed: Sep. 15, 2026].

[14] Vanguard, "Vanguard Total Bond Market ETF (BND)," 2026. [Online]. Available: https://advisors.vanguard.com/investments/products/bnd/vanguard-total-bond-market-etf. [Accessed: Sep. 15, 2026].
