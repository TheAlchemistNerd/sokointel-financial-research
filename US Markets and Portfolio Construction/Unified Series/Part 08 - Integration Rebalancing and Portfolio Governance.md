# Part 08: Integration, Rebalancing and Portfolio Governance

*US Markets and Portfolio Construction, unified nine-part series. Research edition: 15 September 2026.*

A portfolio becomes a durable policy when allocations, cash flows, rebalancing triggers and decision rights are documented together.

## Series scope

The preceding chapters separate the major decisions involved in an illustrative U.S. portfolio. SPYM represents broad U.S. large-cap beta, QQQM is a deliberate Nasdaq-100 growth tilt, a semiconductor ETF is a sector overweight rather than a new asset class, and the option wheel is a payoff-modifying equity strategy rather than guaranteed income. The purpose of integration is to make those roles, interactions and limits visible at portfolio level.
...
This article uses an illustrative policy of 60% SPYM, 15% QQQM, 10% semiconductor exposure and 15% wheel/collateral reserve because that structure uses a bounded example while keeping the active sleeves bounded. The percentages are not personalized advice. They are a worked case for the companion workbook.
...
## 19. The boundaries of an integrated portfolio
