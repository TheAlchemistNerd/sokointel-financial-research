# Part 1 — Building the Core: SPYM, QQQM and the Economics of Low-Cost U.S. Equity Beta

## Series scope

A portfolio is easier to manage when every holding has a job. The starting job in this five-part series is **core exposure**: a low-cost, rules-based allocation intended to capture a large share of the long-run return of U.S. listed equities without requiring repeated security selection. The research notes supplied for this project repeatedly converged on two tickers for that job: **SPYM**, which tracks the S&P 500, and **QQQM**, which tracks the Nasdaq-100. This article turns those notes into a source-verified framework and, just as importantly, corrects a stale comparison in the original material.
...
## 4. Correcting the old QQQ versus QQQM argument

The supplied research note correctly focused on QQQM’s lower expense ratio for long-horizon accumulation, but part of its reasoning is now outdated. Older comparisons described QQQ as a unit investment trust with a 0.20% expense ratio. Invesco announced that, effective in December 2025, QQQ was modernized from a UIT into an open-end ETF and its expense ratio was reduced from 0.20% to **0.18%** [6]. QQQM remains at **0.15%** [3].
...
The old QQQ-versus-QQQM argument must be updated: QQQ is no longer a UIT and now carries a 0.18% expense ratio after its December 2025 modernization [6]. QQQM still has the lower stated fee, but benchmark fit and trading requirements should come before a three-basis-point fee difference.
